...
\noindent
However this is not always enough, sometimes we need other techniques. One of the most important ones is the \keyword{method of substitution}\index{method of substitution}, which is really just applying the integral to the chain rule. We know that
\[
...
\begin{remark}
	...
	\]
	but this is \keyword{incorrect}! Indeed $\csc$ is discontinuous at $x = 0$, and we are attempting to compute an infinite area. This is another example of an improper integral.
\end{remark}
...
\begin{example}
	Calculate the positive area bounded by $y = \cos(x)$, $y = 0$, $x = 0$, and $x = 3 \pi / 2$. We have
	\begin{align*}
...
\noindent
Suppose we want to compute the area bounded by $y = f(x)$, $y = g(x)$, $x = a$, and $x = b$. Assume $a < b$ and $f(x) \leq g(x)$ for all $x \in {[a, b]}$.
...
\begin{example}
	...
	\]
	so $x = 2$ and $x = -1$. Since $4 - x^2 \geq x^2 - 2 x$ for $-1 \leq x \leq 2$, we have
	\begin{align*}
