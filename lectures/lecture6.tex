...
\]
to be defined not only for $r \in \Q$, but for all $r$! Note that this is not an equality we prove, but a definition we take. We therefore \emph{define}
\[
	e^x = \exp(x)
\]
for all $x \in \R$.
...
\begin{proof}
	By defintion $y = e^x$ is equivalent with $x = \ln(y)$. If we differentiate this implicitly we get
	\[
...
\noindent
The fact that $e^x$ is now defined for all $x$ helps us to define arbitrary exponentials $a^x$, for $a > 0$. For rational $r$ we have
\[
	\ln(a^r) = r \ln(a),
\]
...
\begin{proof}
	\fakeitem{1} Let $x > 1$, $a > 0$, and $s = a / 2$. Since $\ln(x^s) = s \ln(x)$, our above result $\ln(x) \leq x - 1$ gives us $\ln(x^s) = s \ln(x) \leq x^s - 1 < x^s$, which dividing by $s$ produces
	\[
		0 < \ln(x) < \frac{x^s}{s}.
	\]
	...
	\fakeitem{2} We use \fakeitemref{1} and substitute $x = 1/t$. Then as $x \to 0^+$ we have $t \to \infty$, whereby
	\[
...
\]
despite $\sin(x) \to 0$ and $x \to 0$ as $x \to 0$. We call $\sin(x) / x$ an \keyword{intermediate form}\index{intermediate form} of the type ``$0 / 0$'' or $[0 / 0]$, depending on the textbook.
